---
name: gate
description: Run the full auto-gate pipeline on a batch — Scout → Gate Dispatcher → Fix → Re-verify → Sentinel
user_invocable: true
---
...
Dispatch `agents/gate-dispatcher.md --build` (Pierce + Mara + Kehinde) against the batch's route.
Returns consolidated findings.

### Step 4 — Additional Gates
Read the project's persona gates config for this batch.
If additional personas required, dispatch `agents/gate-dispatcher.md --manifest` to auto-route.
...
### Step 6 — Re-Verify
Re-dispatch the gate on the fixed code.
